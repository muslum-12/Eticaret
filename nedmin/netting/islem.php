...
if (isset($_POST['logoduzenle'])) {
	...
	if ($_FILES['ayar_logo']['size']>1048576) {
		...
		Header("Location:../production/logo-ayar.php?durum=dosyabuyuk");
	...
	if (in_array($ext, $izinli_uzantilar) === false) {
		...
		Header("Location:../production/logo-ayar.php?durum=formathata");
...
if ($_GET['bankalogosil']=="ok") {

	islemkontrol();
	
	$sil=$db->prepare("DELETE from bankafoto where bankalogo_id=:bankalogo_id");
	$kontrol=$sil->execute(array(
		'bankalogo_id' => $_GET['bankalogo_id']
	));

	if ($kontrol) {

		$resimsilunlink=$_GET['banka_log'];
		unlink("../../$resimsilunlink");

		Header("Location:../production/banka-logo.php?durum=ok");

	} else {

		Header("Location:../production/banka-logo.php?durum=no");
	}
...
if (isset($_POST['bankalogokaydet'])) {



	$uploads_dir = '../../dimg/bankalogo';
	@$tmp_name = $_FILES['banka_log	']["tmp_name"];
	@$name = $_FILES['banka_log	']["name"];
	//resmin isminin benzersiz olması
	$benzersizsayi1=rand(20000,32000);
	$benzersizsayi2=rand(20000,32000);
	$benzersizsayi3=rand(20000,32000);
	...
	$bankalogkaydet=$db->prepare("INSERT INTO bankafoto SET
		banka_log=:banka_log
	
		");
	$insert=$bankalogkaydet->execute(array(
		'banka_log' => $refimgyol
	));
...
if (isset($_POST['magazakaydet'])) {

    $kaydet=$db->prepare("INSERT INTO magazalar SET
		magaza_ad=:magaza_ad,
		magaza_adres=:magaza_adres,
		magaza_il=:magaza_il,
		magaza_konum=:magaza_konum,
		magaza_ilce=:magaza_ilce
		");
	$insert=$kaydet->execute(array(
		'magaza_ad' => $_POST['magaza_ad'],
		'magaza_adres' => $_POST['magaza_adres'],
		'magaza_il' => $_POST['magaza_il'],
		'magaza_konum' => $_POST['magaza_konum'],
		'magaza_ilce' => $_POST['magaza_ilce'],
	));

	if ($insert) {

		Header("Location:../production/magaza.php?durum=ok");

	} else {

		Header("Location:../production/magaza.php?durum=no");
	}
...
if (isset($_POST['magazaduzenle'])) {

	$magaza_id=$_POST['magaza_id'];

	$kaydet=$db->prepare("UPDATE magazalar SET
		...
		magaza_konum=:magaza_konum,
		magaza_ilce=:magaza_ilce
		WHERE magaza_id={$_POST['magaza_id']}");
	$update=$kaydet->execute(array(
		'magaza_ad' => $_POST['magaza_ad'],
		'magaza_adres' => $_POST['magaza_adres'],
		'magaza_il' => $_POST['magaza_il'],
	...
	if ($update) {
		...
		Header("Location:../production/magaza-duzenle.php?magaza_id=$magaza_id&durum=ok");

	} else {

		Header("Location:../production/magaza-duzenle.php?magaza-duzenle=$magaza-duzenle&durum=no");
	}
...
if ($_GET['magazasil']=="ok") {
	
	$sil=$db->prepare("DELETE from magazalar where magaza_id=:magaza_id");
	$kontrol=$sil->execute(array(
		'magaza_id' => $_GET['magaza_id']
	));
